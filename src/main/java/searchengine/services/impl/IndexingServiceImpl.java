...
public class IndexingServiceImpl implements IndexingService {
	...
	public ResponseEntity<Object>  indexingStart() {
		...
		SINGLE_TASK = new Thread(() -> indexingActions.startFullIndexing(siteEntities), "indexing-thread");
		SINGLE_TASK.start();
	...
	public ResponseEntity<Object> indexingPageStart(String url) {
		...
		if (url == null || url.equals(""))
			return new ResponseEntity<>(new BadRequest(false, "Индексацию запустить не удалось. Пустой поисковый запрос или список сайтов"),
					HttpStatus.BAD_REQUEST);

		SiteEntity siteEntity = schemaActions.partialInit(url);
		if (siteEntity == null) return new ResponseEntity<>(new BadRequest(false, "Данная страница находится за пределами сайтов,указанных в конфигурационном файле"),
				HttpStatus.BAD_REQUEST);
